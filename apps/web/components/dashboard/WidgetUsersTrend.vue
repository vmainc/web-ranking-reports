<template>
  ...
  >
    ...
    <div v-else-if="loaded" ref="chartEl" class="h-[260px] w-full" />
...
function usersSeriesForDays(isoDays: string[], rows: Array<{ date: string; activeUsers: number }>): number[] {
  const map = new Map<string, number>()
  for (const row of rows) map.set(normalizeReportDate(row.date), row.activeUsers)
...
async function load() {
  ...
  try {
    ...
    const res = await $fetch<{ rows: Array<{ date: string; activeUsers: number }> }>('/api/google/analytics/report', {
      query: { siteId: props.siteId, startDate, endDate },
      headers,
    })
    const values = usersSeriesForDays(mainDays, res.rows ?? [])
    ...
    if (showCompare) {
      ...
      const prevRes = await $fetch<{ rows: Array<{ date: string; activeUsers: number }> }>('/api/google/analytics/report', {
        query: { siteId: props.siteId, startDate: comp.startDate, endDate: comp.endDate },
        headers,
      })
      compareValues = usersSeriesForDays(compareDays, prevRes.rows ?? [])
    }
        ...
        ? [
            {
              ...
              symbol: 'none',
              lineStyle: { width: 2.5 },
              itemStyle: { color: DV.green },
              areaStyle: dv ? { color: 'rgba(34, 197, 94, 0.14)' } : { color: 'rgba(37, 99, 235, 0.12)' },
            },
            {
