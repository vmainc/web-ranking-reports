    ...
    <div class="flex items-start justify-between gap-3">
      <div class="flex min-w-0 flex-1">
        ...
        >
          ...
          <svg v-if="isGoogle(provider)" class="h-full w-full" viewBox="0 0 24 24" fill="none" aria-hidden="true">
          ...
          <!-- Bing Webmaster -->
          <svg v-else-if="isBingWebmaster(provider)" class="h-full w-full" viewBox="0 0 24 24" aria-hidden="true">
            <path fill="#008373" d="M5 3v18l5.5-8 4.5 6 4-5 2 2V3H5zm2 2h8v4.6l-2.2 3.2L9 8.4 7 11V5z"/>
          </svg>
          <!-- WooCommerce -->
          <svg v-else-if="isWooCommerce(provider)" class="h-full w-full" viewBox="0 0 24 24" aria-hidden="true">
            <path fill="#96588A" d="M19 6h-2c0-2.76-2.24-5-5-5S7 3.24 7 6H5c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm-7-3c1.66 0 3 1.34 3 3h-6c0-1.66 1.34-3 3-3zm7 17H5V8h2v2c0 .55.45 1 1 1s1-.45 1-1V8h6v2c0 .55.45 1 1 1s1-.45 1-1V8h2v12z"/>
          </svg>
...
const providerLogoUrl = computed(() => {
  ...
  if (GOOGLE_PROVIDERS.includes(p as (typeof GOOGLE_PROVIDERS)[number])) return true
  if (p === 'bing_webmaster' || p === 'woocommerce') return true
  return null
})
