  ...
  >
    <div v-if="variant !== 'actions'" class="flex items-start justify-between gap-2.5">
      <div class="flex min-w-0 flex-1">
        <!-- Brand marks via Simple Icons (CC0): https://github.com/simple-icons/simple-icons — CDN for crisp SVGs -->
        <div
          class="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-white p-0.5 ring-1 ring-surface-200"
          :class="effectiveConnected ? 'ring-green-200/80' : ''"
        >
          <img
            v-if="brandIconCdn"
            :src="brandIconCdn"
            ...
            loading="lazy"
          />
          <!-- Bing Webmaster (no Simple Icons CDN slug; stylized mark inspired by Bing green) -->
          <svg v-else-if="isBingWebmaster(provider)" class="h-6 w-6" viewBox="0 0 24 24" aria-hidden="true">
            <path fill="#008373" d="M5 3v18l5.5-8 4.5 6 4-5 2 2V3H5zm2 2h8v4.6l-2.2 3.2L9 8.4 7 11V5z" />
          </svg>
          <!-- Google account / other -->
          <svg v-else-if="isGoogle(provider)" class="h-6 w-6" viewBox="0 0 24 24" fill="none" aria-hidden="true">
...
import { BRAND_ICON_BY_PROVIDER, brandIconCdnUrl } from '~/utils/integrationBrandIcons'
...
const brandIconCdn = computed(() => {
  const spec = BRAND_ICON_BY_PROVIDER[props.provider]
  return spec ? brandIconCdnUrl(spec) : ''
})
...
const statusLabel = computed(() => getStatusLabel(effectiveStatus.value as 'disconnected' | 'pending' | 'connected' | 'error'))
