<template>
  <div class="space-y-6">
    <section class="rounded-xl border border-surface-200 bg-white shadow-sm">
      <div class="flex flex-col gap-3 border-b border-surface-200 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="mt-0.5 text-sm text-surface-500">Email builder reports on your chosen cadence with a PDF attachment.</p>
        </div>
        <button
          v-if="!showCreateForm"
          type="button"
      ...
      <div v-else-if="!filteredSchedules.length" class="px-6 py-12 text-center">
        <p class="text-surface-500">No automated reports yet.</p>
        <p class="mt-1 text-sm text-surface-500">Create a schedule to send a builder report automatically.</p>
        <button
          v-if="!showCreateForm"
          type="button"
          class="mt-4 inline-flex items-center justify-center rounded-lg bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-500"
          @click="showCreateForm = true"
        >
          + New automated report
        </button>
      ...
      <div v-else>
        <p v-if="reports.length > 1" class="border-b border-surface-200 px-6 py-3 text-sm text-surface-600">
          <label class="mr-2 font-medium text-surface-700">Filter by report</label>
          <select v-model="reportFilter" class="mt-1 rounded-lg border border-surface-300 bg-white px-3 py-2 text-sm text-surface-900 sm:mt-0">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-surface-200">
            ...
            <tbody class="divide-y divide-surface-200 bg-white">
              <tr v-for="row in filteredSchedules" :key="row.id" class="transition-colors hover:bg-surface-50/50">
                <td class="px-6 py-4 text-sm font-medium text-surface-900">{{ row.expand?.report ? reportDisplayName(row.expand.report) : '—' }}</td>
                <td class="px-6 py-4 text-sm text-surface-600">{{ row.expand?.site?.name ?? '—' }}</td>
                ...
                <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                  <span class="inline-flex flex-wrap items-center justify-end gap-x-2 gap-y-1">
                    ...
                    <span v-if="scheduleReportId(row)" class="text-surface-300">|</span>
                    <button type="button" class="font-medium text-primary-600 hover:underline" :disabled="mutatingId === row.id" @click="openEdit(row)">
                      Edit
                    </button>
                    ...
                    <button
                      ...
                      type="button"
                      class="font-medium text-surface-600 hover:underline"
      ...
      <p v-if="error" class="border-t border-surface-200 px-6 py-3 text-sm text-red-600">{{ error }}</p>
    </section>

    <section v-if="showCreateForm" class="schedule-form-panel rounded-xl border border-surface-200 bg-white p-6 shadow-sm">
      <div class="mb-4 flex items-start justify-between gap-3">
        ...
        <button
          type="button"
          class="shrink-0 text-sm font-medium text-surface-500 hover:text-surface-900"
          aria-label="Close"
    ...
    <Teleport to="body">
      ...
      >
        <div class="schedule-form-panel w-full max-w-md rounded-xl border border-surface-200 bg-white p-6 shadow-2xl" @click.stop>
    ...
    <Teleport to="body">
      ...
      >
        <div class="schedule-form-panel w-full max-w-sm rounded-xl border border-surface-200 bg-white p-6 shadow-2xl" @click.stop>
