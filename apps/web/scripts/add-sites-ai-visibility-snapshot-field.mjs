#!/usr/bin/env node
/**
 * Add optional JSON field `ai_visibility_snapshot` to PocketBase `sites` (cached LLM Mentions snapshot).
 *
 *   cd apps/web && POCKETBASE_ADMIN_EMAIL=... POCKETBASE_ADMIN_PASSWORD=... node scripts/add-sites-ai-visibility-snapshot-field.mjs
 */
...
async function main() {
  ...
  if (schemaArray.some((f) => f && f.name === 'ai_visibility_snapshot')) {
    console.log('Sites collection already has "ai_visibility_snapshot". Nothing to do.')
    process.exit(0)
  }
  schemaArray.push({ name: 'ai_visibility_snapshot', type: 'json', required: false })
  await pb.collections.update(sites.id, { schema: schemaArray })
  console.log('Added "ai_visibility_snapshot" (json) to sites.')
}
