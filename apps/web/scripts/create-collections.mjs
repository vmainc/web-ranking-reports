...
async function main() {
  ...
  if (!hasIntegrations) {
    const integrationsBody = {
      ...
      schema: [
        ...
        { name: 'provider', type: 'select', required: true, options: { values: ['google_analytics', 'google_search_console', 'lighthouse', 'google_business_profile', 'google_ads', 'google_local_services_ads', 'woocommerce', 'bing_webmaster', 'cloudflare'], maxSelect: 1 } },
  ...
  if (!hasCrmClients) {
    const crmClientsBody = {
      ...
      schema: [
        { name: 'user', type: 'relation', required: true, options: { collectionId: usersCol.id, maxSelect: 1, cascadeDelete: true } },
        { name: 'name_prefix', type: 'text', required: false, options: { max: 30 } },
        { name: 'first_name', type: 'text', required: false, options: { max: 120 } },
        { name: 'last_name', type: 'text', required: false, options: { max: 120 } },
        { name: 'name', type: 'text', required: true, options: { min: 1, max: 255 } },
        { name: 'email', type: 'text', required: false, options: { max: 255 } },
        { name: 'phone', type: 'text', required: false, options: { max: 100 } },
        { name: 'business_phone', type: 'text', required: false, options: { max: 100 } },
        { name: 'cell_phone', type: 'text', required: false, options: { max: 100 } },
