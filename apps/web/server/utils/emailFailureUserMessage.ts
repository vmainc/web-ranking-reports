...
export function emailFailureUserMessage(err: unknown, kind: 'member' | 'client' | 'report'): string {
  ...
  if (
    ...
    return `${created} On the server, add SMTP_USER and SMTP_PASSWORD to infra/.env (same mailbox as PocketBase → Settings → Mailer), then from the repo root: docker compose --project-directory "$PWD/infra" --env-file "$PWD/infra/.env" -f "$PWD/infra/docker-compose.yml" up -d web (or ./infra/deploy.sh). Locally: apps/web/.env and restart dev. PocketBase’s API never exposes that password. They can still use Forgot password on the login page.`
  }
