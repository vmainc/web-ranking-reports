...
export default defineEventHandler(async (event) => {
  ...
  const key = cacheKey(siteId, 'countries', { limit: String(limit), range })
  const cached = getCached<{ rows: Array<{ country: string; users: number; sessions: number; views: number }> }>(key)
  ...
  let rows: Array<{ country: string; users: number; sessions: number; views: number }> = []
  try {
