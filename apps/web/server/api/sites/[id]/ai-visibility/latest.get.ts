...
/**
 * Cached AI visibility snapshot from the site record.
 ...
 * Optional `maxKeywords` caps rank-tracked keywords included (default 5, max 5).
 */
  ...
  const maxAgeDays =
    typeof maxAgeDaysRaw === 'string' && maxAgeDaysRaw.trim()
      ...
      : typeof maxAgeDaysRaw === 'number'
        ...
        : 0
  const maxKeywordsRaw = query.maxKeywords
  const maxKeywords =
    typeof maxKeywordsRaw === 'string' && maxKeywordsRaw.trim()
      ? Math.max(0, Number(maxKeywordsRaw) || 0)
      : typeof maxKeywordsRaw === 'number'
        ? Math.max(0, maxKeywordsRaw)
        : undefined

  return await resolveSiteAiVisibilitySnapshot(pb, siteId, row, {
    refresh,
    fetchIfMissing,
