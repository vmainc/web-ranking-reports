...
export default defineEventHandler(async (event) => {
  ...
  try {
    const row = await pb.collection('app_settings').getFirstListItem<{ value: { api_key?: string; model?: string } }>('key="claude_api"')
    const value = row?.value ?? {}
    return { api_key: value.api_key ?? '', model: value.model ?? '' }
  } catch {
