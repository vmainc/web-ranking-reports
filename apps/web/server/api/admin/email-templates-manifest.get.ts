...
import { EMAIL_TEMPLATE_CATALOG } from '~/server/utils/emailTemplateCatalog'
...
function normalizeAuthTemplate(v: unknown): EmailTemplatePayload {
...
export default defineEventHandler(async (event) => {
  ...
  const auth = {
    verification: normalizeAuthTemplate(opt.verificationTemplate),
    passwordReset: normalizeAuthTemplate(opt.resetPasswordTemplate),
    emailChange: normalizeAuthTemplate(opt.confirmEmailChangeTemplate),
  }
  ...
  return {
    catalog: EMAIL_TEMPLATE_CATALOG,
    authCollectionId: col.id,
    auth,
    transactional,
  }
})
