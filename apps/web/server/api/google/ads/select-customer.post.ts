...
export default defineEventHandler(async (event) => {
  ...
  const body = (await readBody(event).catch(() => ({}))) as { siteId?: string; customer_id?: string; customer_name?: string }
  ...
  const config = { ...anchor.config_json, ads_customer_id: customerId, ads_customer_name: customerName || customerId }
