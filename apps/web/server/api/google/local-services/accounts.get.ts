...
type AdsCustomerItem = { resourceName: string; customerId: string; name: string; managerId?: string }
...
export default defineEventHandler(async (event): Promise<{ customers: AdsCustomerItem[] }> => {
  ...
  const listRes = await $fetch<{ resourceNames?: string[] }>('https://googleads.googleapis.com/v20/customers:listAccessibleCustomers', {
    method: 'GET',
    headers: {
  ...
  async function getCustomerDescriptiveName(customerId: string): Promise<string | null> {
    try {
      ...
      const name =
        (row?.customer as { descriptiveName?: string })?.descriptiveName?.trim() ||
  ...
  for (const customerId of accessibleIds) {
    ...
    try {
      ...
      for (const row of results) {
        const client =
          (row as { customerClient?: { id?: string | number; descriptiveName?: string }; customer_client?: { id?: string | number; descriptive_name?: string } }).customerClient ??
        ...
        seen.add(id)
        const name =
          (client as { descriptiveName?: string }).descriptiveName?.trim() ||
