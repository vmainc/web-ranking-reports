...
/** OAuth query params can arrive as string | string[] from proxies or duplicate keys — normalize. */
...
export default defineEventHandler(async (event) => {
  ...
  const appUrl = String(
    (config.appUrl as string) || (config.public as { appUrl?: string }).appUrl || 'https://webrankingreports.com',
  ).replace(/\/+$/, '')
  ...
  if (errorParam) {
    console.error('Google callback: OAuth error', errorParam, singleQueryParam(query.error_description))
  ...
  try {
    ...
    // PocketBase ClientResponseError (e.g. 400 validation) would otherwise render as a bare Nuxt error page on this URL.
    console.error('Google callback: failed after state verification', e)
    return sendRedirect(event, siteErrorRedirect(appUrl, payload, 'error'))
  }
})
