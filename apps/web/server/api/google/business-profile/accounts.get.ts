...
export default defineEventHandler(async (event) => {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      const stale = gbpAccountsCache.get(cacheKey)
      if (stale?.data.accounts?.length) {
        setResponseStatus(event, 200)
        return { ...stale.data, rateLimited: true }
      }
      // Retry once after cooldown so "wait a minute" can succeed in one request
      ...
      if (!res.ok) {
        ...
        throw createError({ statusCode: res.status, message: `Business Profile API: ${res.status} ${retryText.slice(0, 200)}` })
      }
      // fall through to parse and cache
    } else {
      if (res.status === 403) {
