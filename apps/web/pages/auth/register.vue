<template>
  <div class="rounded-2xl border border-surface-200 bg-white p-8 shadow-card">
    ...
    <template v-else>
      <h1 class="mb-2 text-xl font-semibold text-surface-900">Sign-up closed</h1>
      <p class="mb-6 text-sm leading-relaxed text-surface-600">
        We are not accepting new accounts right now. If you already have an account, sign in below. Otherwise
        <NuxtLink to="/contact" class="font-medium text-primary-600 hover:text-primary-500">contact us</NuxtLink>
        for access.
      </p>
      <NuxtLink
        to="/auth/login"
        class="inline-flex w-full items-center justify-center rounded-lg bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-500"
      >
        Sign in
      </NuxtLink>
      <NuxtLink
        to="/contact"
        class="mt-3 inline-flex w-full items-center justify-center rounded-lg border border-surface-200 px-4 py-2.5 text-sm font-semibold text-surface-800 transition hover:bg-surface-50"
      >
        Contact us
      </NuxtLink>
    </template>
...
const { registrationEnabled } = useSignupAccess()
