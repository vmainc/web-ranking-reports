  ...
  <div class="mx-auto max-w-6xl px-4 py-8 sm:px-6">
    <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-surface-500">Only clients (not leads) appear here. Link a client to a site in their profile to see integration status below.</p>
      </div>
    ...
    <section class="rounded-xl border border-surface-200 bg-white shadow-sm">
      ...
      <p class="px-6 py-2 text-sm text-surface-500">Same metrics as on each site’s Settings → What’s connected. Only contacts with status Client are listed. Link a client to a site in their profile to see status here.</p>

      <div v-if="pending" class="px-6 py-12 text-center text-surface-500">Loading…</div>
      <div v-else-if="!rows.length" class="px-6 py-12 text-center text-sm text-surface-500">No clients yet. Only contacts with status Client appear in this table. Add clients in Contacts and link them to a site to see onboarding status.</div>
